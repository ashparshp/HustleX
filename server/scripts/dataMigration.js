...
const ActivityTracker = mongoose.model('ActivityTracker') || { schema: mongoose.Schema({}) };
...
const migrateData = async () => {
  try {
    ...
    let adminUser = await User.findOne({ email: process.env.ADMIN_EMAIL || 'admin@example.com' });
    ...
    const categoryTypes = ['working-hours', 'skills', 'schedule', 'timetable', 'goals'];
    ...
    for (const type of categoryTypes) {
      ...
      switch (type) {
        case 'working-hours':
          ...
          categories = [...new Set(workingHours.map(wh => wh.category).filter(Boolean))];
          break;
          
        case 'skills':
          const skills = await Skill.find();
          categories = [...new Set(skills.map(skill => skill.category).filter(Boolean))];
          break;
          ...
          schedules.forEach(schedule => {
            if (schedule.items && Array.isArray(schedule.items)) {
              schedule.items.forEach(item => {
                if (item && item.category) scheduleCategories.add(item.category);
              });
            }
          });
        ...
        case 'timetable':
          try {
            ...
            const activityTrackerCollection = mongoose.connection.db.collection('activitytrackers');
            if (activityTrackerCollection) {
              const activityDocs = await activityTrackerCollection.find({}).toArray();
              if (activityDocs.length > 0 && activityDocs[0].defaultActivities) {
            ...
            console.log('No ActivityTracker collection found, using default categories');
        ...
        case 'goals':
          ...
          categories = [...new Set(contests.map(contest => contest.platform).filter(Boolean))];
          if (categories.length === 0) {
            categories = ['LeetCode', 'CodeChef', 'CodeForces', 'HackerRank'];
          }
          break;
      }
    ...
    console.log(`Migrated ${contestCount} contests`);
    
    // Migrate LeetCode stats
    console.log('Migrating LeetCode stats...');
    const leetCodeStats = await LeetCode.find();
    let statCount = 0;
    
    for (const stat of leetCodeStats) {
      // Check if already migrated (based on timestamp)
      const existingLeetCode = await LeetCode.findOne({
        user: adminUser._id,
        createdAt: stat.createdAt
      });
      
      if (!existingLeetCode) {
        const newStat = new LeetCode({
          ...stat.toObject(),
          _id: undefined, // Create new ID
          user: adminUser._id
        });
    ...
    console.log(`Migrated ${statCount} LeetCode stat records`);
        ...
        if (activityDocs.length > 0) {
          ...
          if (!existingTimetable) {
            ...
            const timetable = new Timetable({
              ...
              isActive: true,
              currentWeek: {
                weekStartDate: activityDoc.currentWeek?.weekStartDate || new Date(),
                weekEndDate: activityDoc.currentWeek?.weekEndDate || new Date(new Date().setDate(new Date().getDate() + 6)),
                activities: activityDoc.currentWeek?.activities || [],
                overallCompletionRate: activityDoc.currentWeek?.overallCompletionRate || 0
              },
